...
**Status:** Approved direction / v1.4.1 refinement preview
...
## Refinement lock — v1.4.1

- The master homepage first viewport represents all three lines visually; Sambal AYA stays the current hero product but does not dominate the master-brand definition.
- Line accent color names remain internal design tokens, not public marketing labels.
- Each line page includes sibling-line discovery and a clear route back to AYA RAOS.
- QR source parameters can continue into product-detail navigation for future measurement without activating analytics.
- Product detail always identifies the owning line and links back to its line gateway.
- Browser QA on the four target viewports remains mandatory before commit/push approval.
