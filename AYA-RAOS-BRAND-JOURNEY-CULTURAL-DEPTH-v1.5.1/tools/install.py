...
PACKAGE = 'AYA-RAOS-BRAND-JOURNEY-CULTURAL-DEPTH-v1.5.1'
ALLOWED_PREFIXES = ('feature/', 'preview/')
BACKUP_PREFIX = '.aya-raos-backup-brand-v151-'
...
def main():
    ap = argparse.ArgumentParser(description='Install AYA RAOS Brand Journey & Cultural Depth v1.5.1 for preview only.')
    ...
    # Refuse unrelated tracked changes. Only package payload paths may already differ.
    ...
    # Validate package checksums and preimages. This package targets the approved main v1.5.0 baseline or already-final v1.5.1 payload.
    states_seen = set()
    already_final = 0
    for item in items:
        rel = item['path']
        ...
        if target.exists():
            ...
            if current not in allowed:
                ...
                print('File bukan baseline main-v1.5.0 yang dikenali dan bukan final v1.5.1.')
    ...
    print('Gate    : Brand journey, cultural depth, readability, 3 line pages, Catalog, Product Detail, Cart, Business, Testimonial/Share.')
    print('DB      : tidak ada migration baru dan tidak ada eksekusi database.')
