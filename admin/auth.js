...
async function verifyAdmin(session){
  ...
  return{
    ok:true,
    user:{user_id:data.user_id,display_name:data.display_name,email:session.user.email||''},
    roles:roleRows,
    functionKeys:assignableFunctions,
    systemOnlyGranted
  };
}
async function enter(session){
  ...
  try{
    ...
    if(!v.ok){await sb.auth.signOut();show('login');message('loginError',v.msg);return;}
  ...
  }catch(e){await sb.auth.signOut();show('login');message('loginError',`Admin access gagal diverifikasi: ${classify(e)}`);}
}
async function init(){
  const recovery=window.location.hash.includes('type=recovery');
  if(recovery)show('reset');
  const{data}=await sb.auth.getSession();
  if(data?.session&&!recovery)await enter(data.session);else if(!recovery)show('login');

  $('loginForm')?.addEventListener('submit',async event=>{
    event.preventDefault();message('loginError','');
    ...
    const btn=$('loginForm')?.querySelector('button[type="submit"]');if(btn)btn.disabled=true;
    try{const{data,error}=await sb.auth.signInWithPassword({email,password});if(error)throw error;await enter(data.session);}
    catch(e){message('loginError',classify(e));}
    finally{if(btn)btn.disabled=false;}
  });

  $('forgotPasswordBtn')?.addEventListener('click',async()=>{
    const email=$('loginEmail')?.value.trim();message('loginError','');
    if(!email){message('loginError','Masukkan email Admin terlebih dahulu.');$('loginEmail')?.focus();return;}
    const redirectTo=`${window.location.origin}/admin/`;
    const{error}=await sb.auth.resetPasswordForEmail(email,{redirectTo});
    if(error)message('loginError',classify(error));else message('loginError','Jika email terdaftar, instruksi reset password telah dikirim.','success');
  });

  $('resetForm')?.addEventListener('submit',async event=>{
    event.preventDefault();message('resetError','');
    ...
    show('login');message('loginError','Password berhasil diperbarui. Silakan login kembali.','success');
  });
