...
const loadSupabaseFallback=()=>{if(window.supabase?.createClient)return Promise.resolve(true);return new Promise(resolve=>{const s=document.createElement('script');s.src='https://unpkg.com/@supabase/supabase-js@2.111.0/dist/umd/supabase.min.js';s.async=false;s.onload=()=>resolve(!!window.supabase?.createClient);s.onerror=()=>resolve(false);document.head.appendChild(s);});};
...
async function bootstrap(){
  ...
  if(!ready||!cfg.url||!cfg.publishableKey){message('loginError','Supabase Admin belum siap. Silakan muat ulang halaman.');window.dispatchEvent(new Event('aya:admin-auth-failed'));return;}
  ...
  async function init(){
    ...
    form?.addEventListener('submit',async event=>{event.preventDefault();message('loginError','');const email=$('loginEmail')?.value.trim()||'';const password=$('loginPassword')?.value||'';if(!email||!password){message('loginError','Email dan password wajib diisi.');return;}const btn=form.querySelector('button[type="submit"]');if(btn)btn.disabled=true;try{const{data,error}=await sb.auth.signInWithPassword({email,password});if(error)throw error;await enter(data.session);}catch(e){message('loginError',classify(e));}finally{if(btn)btn.disabled=false;}});
  ...
  // All Admin DOM exists because auth.js is loaded at the end of index.html. Bind immediately; do not wait for DOMContentLoaded on mobile.
  init();
}
bootstrap().catch(e=>{ensureHiddenStyles();message('loginError',`Admin Auth gagal dimuat: ${classify(e)}`);window.dispatchEvent(new Event('aya:admin-auth-failed'));});
})();